<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Lista de Trayectos</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-4 gap-4">
                    <div class="flex gap-4">
                        <a href="{{ route('trayectos.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg"></i>
                            Crear Trayecto
                        </a>
                        <a href="{{ route('trayectos.index', ['ver_inactivas' => !$mostrarInactivas]) }}"
                ...
                <table class="w-full table-auto border-collapse border border-gray-200" id="mainTable">
                    ...
                    <tbody id="tableBody">
                        @if ($trayectos->isEmpty())
                            <tr>
                                <td colspan="10" class="border px-4 py-2 text-center">No hay trayectos registrados
                                </td>
                            </tr>
                        @endif
                        @foreach ($trayectos as $trayecto)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="border border-gray-300 px-4 py-2">{{ $trayecto->codigo }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $trayecto->nombre }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $trayecto->descripcion }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $trayecto->pnf->nombre }}</td>
                                {{--                                 <td class="border border-gray-300 px-4 py-2">
                                    {{ $trayecto->estatus == '1' ? 'Activo' : 'Inactivo' }}</td> --}}
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('trayectos.edit', $trayecto) }}"
                                        class="btn btn-warning inline-flex items-center gap-1 px-3 py-1 rounded hover:bg-yellow-400 transition">
                                        <i class="bi bi-pencil"></i> Editar</a>
                                    <form action="{{ route('trayectos.destroy', $trayecto) }}" method="POST"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.delete-form').forEach(function(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: "¿Estás seguro?",
                        text: "¡Vas a Desactivar este Trayecto!",
                        icon: "warning",
