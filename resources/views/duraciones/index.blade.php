<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Lista de Duraciones</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-4 gap-4">
                    <div class="flex gap-4">
                        <a href="{{ route('duraciones.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg"></i>
                            Crear Duración
                        </a>

                        <a href="{{ route('duraciones.index', ['ver_inactivas' => !$mostrarInactivas]) }}"
                ...
                <table class="w-full table-auto border-collapse border border-gray-200" id="mainTable">
                    ...
                    <tbody id="tableBody">
                        @if ($duraciones->isEmpty())
                            <tr>
                                <td colspan="10" class="border px-4 py-2 text-center">No hay Duraciones registradas.
                                </td>
                            </tr>
                        @endif
                        @foreach ($duraciones as $duracion)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="border border-gray-300 px-4 py-2">{{ $duracion->nombre }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $duracion->descripcion }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('duraciones.edit', $duracion) }}" class="btn btn-warning"><i
                                            class="bi bi-pencil"></i> Editar</a>
                                    <form action="{{ route('duraciones.destroy', $duracion) }}" method="POST"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.delete-form').forEach(function(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: "¿Estás seguro?",
                        text: "¡Vas a Desactivar esta Duración!",
                        icon: "warning",
