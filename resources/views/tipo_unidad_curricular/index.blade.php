<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Lista de Tipos de Unidad Curricular</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-4 gap-4">
                    <div class="flex gap-4">
                        <a href="{{ route('tipo_unidad_curricular.create') }}" class="btn btn-primary"><i
                                class="bi bi-plus-lg"></i> Crear Tipo de Unidad Curricular
                        </a>

                        <a href="{{ route('tipo_unidad_curricular.index', ['ver_inactivas' => true]) }}"
                ...
                <table class="w-full table-auto border-collapse border border-gray-200" id="mainTable">
                    ...
                    <tbody id="tableBody">
                        @if($tipoUnidadesCurriculares->isEmpty())
                            <tr>
                                <td colspan="10" class="border px-4 py-2 text-center">No existen tipos de unidades curriculares registradas.</td>
                            </tr>
                        @endif
                        @foreach ($tipoUnidadesCurriculares as $tipoUnidadCurricular)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="border border-gray-300 px-4 py-2">{{ $tipoUnidadCurricular->nombre }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $tipoUnidadCurricular->descripcion }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('tipo_unidad_curricular.edit', $tipoUnidadCurricular) }}"
                                    class="btn btn-warning inline-flex items-center gap-1 px-3 py-1 rounded hover:bg-yellow-400 transition">
                                    <i class="bi bi-pencil"></i>Editar</a>
                                    <form action="{{ route('tipo_unidad_curricular.destroy', $tipoUnidadCurricular) }}"
                                        method="POST" class="inline-block ml-2 delete-form">
                                        @csrf
                                        @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.delete-form').forEach(function(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: "¿Estás seguro?",
                        text: "¡Vas a Desactivar este tipo de Unidad Curricular!",
                        icon: "warning",
