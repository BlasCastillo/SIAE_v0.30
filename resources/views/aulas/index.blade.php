<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Lista de Aulas</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6">
                ...
                <div class="flex justify-between items-center mb-4 gap-4">
                    <div class="flex gap-4">
                        <a href="{{ route('aulas.create') }}" class="btn btn-primary"><i class="bi bi-plus-lg"></i> Crear Aula</a>
                ...
                <table class="w-full table-auto border-collapse border border-gray-200" id="mainTable">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="border border-gray-300 px-4 py-2 text-left">Capacidad</th>
                            <th class="border border-gray-300 px-4 py-2 text-left">Tipo de Aula</th>
                    ...
                    <tbody id="tableBody">
                        @if($aulas->isEmpty())
                            <tr>
                                <td colspan="6" class="border px-4 py-2 text-center">No hay aulas registradas.</td>
                            </tr>
                        @endif
                        @foreach($aulas as $aula)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="border border-gray-300 px-4 py-2">{{ $aula->nombre }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $aula->descripcion }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $aula->cantidad }}</td>
                                <td class="border border-gray-300 px-4 py-2">{{ $aula->tipoAulas->nombre }}</td>
                                <td class="border border-gray-300 px-4 py-2">
                                    <a href="{{ route('aulas.edit', $aula) }}" class="btn btn-warning inline-flex items-center gap-1 px-3 py-1 rounded hover:bg-yellow-400 transition">
                                        <i class="bi bi-pencil"></i> Editar
                                    </a>
                                    <form action="{{ route('aulas.destroy', $aula) }}" method="POST" class="inline-block ml-2 delete-form">
                                        @csrf
                                        @method('DELETE')
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('.delete-form').forEach(function(form) {
                form.addEventListener('submit', function(e) {
                    ...
                    Swal.fire({
                        title: "¿Estás seguro?",
                        text: "¡Vas a Desactivar esta aula!",
                        icon: "warning",
