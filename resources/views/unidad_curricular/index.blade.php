<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Lista de Unidades Curriculares</h2>
    </x-slot>
                ...
                <div class="flex justify-between items-center mb-4 gap-4">
                    ...
                    <form method="GET" action="{{ route('unidad_curricular.index') }}"
                        ...
                        <div class="relative flex-grow max-w-md">
                            ...
                            <select name="filter_pnf" id="filter_pnf"
                        ...
                        <div class="relative flex-grow max-w-md">
                            ...
                            <select name="filter_trayecto" id="filter_trayecto"
                                class="w-full pl-4 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition"
                                disabled>
                                <option value="">Todos</option>
                            </select>
...
<script>
    $(document).ready(function() {
        $('#filter_pnf').on('change', function() {
            const pnfId = $(this).val();
            $('#filter_trayecto').prop('disabled', true).empty().append(
                '<option value="">Todos</option>');

            if (pnfId) {
                // Enviar solicitud al nuevo endpoint para obtener los trayectos
                $.get(`/unidad_curricular/trayectos/${pnfId}`, function(data) {
                    $('#filter_trayecto').prop('disabled', false).empty().append(
                        '<option value="">Todos</option>');
                    data.forEach(trayecto => {
                        $('#filter_trayecto').append(
                            `<option value="${trayecto.id}">${trayecto.nombre}</option>`
                        );
                    });
                }).fail(function() {
                    $('#filter_trayecto').prop('disabled', true).append(
                        '<option value="">Error al cargar trayectos</option>');
                });
            }
        });
    });
